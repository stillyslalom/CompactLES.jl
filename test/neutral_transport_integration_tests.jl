...
@testset "neutral H2-D2 Loschmidt diffusion" begin
    ...
    for n in (24, 48)
        ...
        rho0 = P0/(T0*1e-4)
        mass0 = sum(Q[gidx(s, i, 1, 1), 1] for i in 1:n)
        total0 = [sum(Q[gidx(s, i, 1, 1), k] for i in 1:n) for k in 1:2]
        ...
        push!(errors, maximum(1:n) do i
            ...
            abs(Q[gidx(s, i, 1, 1), 1] - exact)/rho0
        end)
        ...
        @test sum(Q[gidx(s, i, 1, 1), 1] for i in 1:n) ≈ mass0 rtol=2e-14
        @test all(isapprox(sum(Q[gidx(s, i, 1, 1), k] for i in 1:n), total0[k];
                           rtol=2e-14) for k in 1:2)
        CL.refresh_primitives!(s, Q)
        @test maximum(abs(s.p[gidx(s, i, 1, 1)] - P0) for i in 1:n) < 2e-9
        @test maximum(abs(s.T_ion[gidx(s, i, 1, 1)] - T0) for i in 1:n) < 2e-11
    end
...
@testset "neutral diffusion respects species identity and model limit" begin
    ...
    @test maximum(abs(Q[gidx(s, i, 1, 1), 1] -
                      Qr[gidx(sr, i, 1, 1), 2]) for i in 1:32) < 2e-9
    @test maximum(abs(Q[gidx(s, i, 1, 1), 2] -
                      Qr[gidx(sr, i, 1, 1), 1]) for i in 1:32) < 2e-9
    ...
    function rhs_with(model)
        ...
        sol = Solver(n_global=(32, 1, 1), L_domain=(L, 1.0, 1.0), bcs=PER,
                     eos=eos, transport=tr, art=ArtParams(enabled=false),
                     filter_interval=0)
...
@testset "physical H2-D2 thermodynamics reaches the conservative RHS" begin
    ...
    @test maximum(abs(s.Y[1][gidx(s, i, 1, 1)] -
                      sr.Y[2][gidx(sr, i, 1, 1)]) for i in 1:32) < 2e-15
    @test maximum(abs(s.Y[2][gidx(s, i, 1, 1)] -
                      sr.Y[1][gidx(sr, i, 1, 1)]) for i in 1:32) < 2e-15
    @test all(isfinite, parent(dQ))
    @test maximum(abs, view(parent(dQ), :, :, :, s.equations.i_energy)) > 0
    species_scale = maximum(abs(dQ[gidx(s, i, 1, 1), k])
                            for i in 1:32, k in 1:2)
    @test maximum(abs(dQ[gidx(s, i, 1, 1), 1] +
                      dQ[gidx(s, i, 1, 1), 2]) for i in 1:32) < 2e-12*species_scale
    for c in (1, 2, s.equations.i_energy)
        values = [dQ[gidx(s, i, 1, 1), c] for i in 1:32]
        @test abs(sum(values)) < 2e-12*sum(abs, values)
    end
    @test maximum(abs(dQ[gidx(s, i, 1, 1), 1] -
                      dQr[gidx(sr, i, 1, 1), 2]) for i in 1:32) < 2e-7
    @test maximum(abs(dQ[gidx(s, i, 1, 1), 2] -
                      dQr[gidx(sr, i, 1, 1), 1]) for i in 1:32) < 2e-7
    @test maximum(abs(dQ[gidx(s, i, 1, 1), s.equations.i_energy] -
                      dQr[gidx(sr, i, 1, 1), sr.equations.i_energy])
                  for i in 1:32) < 2e-7
end
