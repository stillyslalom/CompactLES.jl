...
julia --project=. test/runtests.jl        # 123 testsets, 0 failures
...
The `123 testsets` and the `168/168` are counted by different mechanisms and are
...
the device/adapt testsets); the `123` counts those rows. A failure is
