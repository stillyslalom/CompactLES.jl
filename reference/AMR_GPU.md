...
[Scope boundaries](#scope-boundaries) says otherwise:
...
A GPU port needs exactly that shape: a patch is a natural launch unit with a
...
change, and every later device stage slotted into seams that refactor had
already cut. Sequencing the AMR-shaped refactor first and the kernels second
...
2. **Minimum extent 9 per resolved dimension.** The C8 filter closure is the
   ...
   numbers rather than letting a regrid die mid-run).
...
5. **The bit-exact oracle survives only within a patch.** The
   ...
   style of `test/convergence.jl`. The device backend, by contrast, turned
   out to support a stronger oracle than the plan expected — see
...
7. **Error localization is favorable.** The inverse of a compact LHS decays
   ...
   interface pollution two orders of magnitude and is sized from exactly
   this number.
...
Restriction applies the filter; prolongation deconvolves it — the same
...
and the fold parity variants for free — the ± symmetric closure variants are
exactly the `lo_fold`/`hi_fold` ghost-folding algebra (measured ≤ 1e-13
...
Chertock (2000); Mattsson & Rydin (2022) — implicit SBP on a banded-norm
...
shared-plane averaging, ghost refill, then each patch's own halo exchange. A
rank advancing several patches sequentially is why no cross-patch
...
region — Dirichlet forcing by the coarse solution, which removes the
...
endpoint RHS is *not* free — the next step's stage-1 RHS arrives too late
and restriction invalidates a cached one — so it costs one extra coarse RHS
...
which is what an unrefined run of either level would do and is dt-consistent
whenever `filter_cfl > 0`. Measured: the entropy-wave orders are unchanged
...
- **The interpolation chains distribute by conserved component**, and that
  split was forced by measurement, not taste. The first cut ran the chains
  ...
  same values move instead of being recomputed — serial results are
...
through the same `field(backend, decomp)` / `allocate_state` seam
`CPUBackend` uses.
...
Kernel-argument adaptation is the load-bearing design. A `Vector{A}` or
...
in the species loops), which is why the wrapper exists at all. The gas-model
EOS objects adapt to coefficient mirrors the same way
(`IdealMixtureCoeffs`, `StiffenedGasCoeffs`); `ConservedState` adapts to
itself around the device-side array. Bodies take no `::Type` argument — a
`Type` inside `pointwise!`'s Vararg defeats specialization and turns the
...
reduces with the storage's own `maximum`/`minimum` — exact, so it matches
the fused CPU loop bitwise. Host-staged on purpose, per the I/O rule that
gathers are explicit: geometry fills host mirrors once at setup and uploads;
...
Per-patch streams were planned and are deliberately not implemented, on the
...
scheduling complexity to overlap kernels an in-order queue already
back-to-backs. Revisit when same-level multi-patch reaches device (its
patches are genuinely independent within a stage) or when the reduced solve
moves on-device.
...
drift 1.4e-4 against 7.5e-13, which is why Float32 is not the CPU default.
On device both precision histories reproduce the CPU numbers to every
printed digit, and Float32 runs 1.25× the Float64 device rate under the
deferred launch policy — better than the CPU's 1.10× but far from the clean
2×, because the step is bounded by launch submission and the reduced-solve
fences rather than arithmetic (the G2 FP64/FP32 kernel measurement:
...
1. **Bitwise equality against the CPU solver, over full runs.** The original
   ...
   sharpest gate the device track has. Every device stage was accepted on
...
2. **CPU-side pins without a GPU.** `FORCE_KA` routes ordinary arrays
   ...
   suites hold all of them bitwise against the default paths. What these
   *cannot* catch is anything only real device storage exercises — see the
   lessons below — so the real-GPU bench battery is mandatory per change to
   the device path, not optional.
...
1. **The live inter-level coupling must match the data's provenance.** The
   ...
   belongs only where the matching convolution has actually been applied.
2. **Replication of coupling work scales as rank count times imposition
   ...
   Allgatherv recovered it (49%) while moving the identical values — the
   fix preserved bitwise serial equality, which is the pattern to prefer:
...
3. **KA-CPU equality cannot certify the device path.** Three defects passed
   ...
   real-GPU battery is part of the gate, not a bonus.
...
7. **The launch floor was host round trips, not kernels.** Synchronizing
   ...
   are not the first tool against launch overhead — deleting synchronization
   is — and a fence the algorithm requires (host reduced solve) bounds what
   any stream topology can recover.
...
9. **Restriction must stand off the imposed boundary.** Writing restricted
   ...
   loop.
10. **NaN survives rollback in exactly two places** — the artificial
    coefficient arrays and the low-storage accumulator (0.0·NaN = NaN
    defeats `RKA[1] = 0`) — and the resets are deliberately gated on
    `:nonfinite` failures because stale *finite* coefficients are a
    measured part of retry recovery.
...
What errors at setup rather than failing downstream, and the reason:
...
- The artificial-property sensors are built per patch with closed-edge
  clamping at interfaces — documented, not measured to matter at the gates
  run so far.

## Roadmap

In rough priority order. Each item names the measurement or event that
should trigger it; nothing here is worth building speculatively.
...
3. **Same-level multi-patch on device.** Stage the interface-record
   ...
   patches are genuinely independent within an RK stage.
...
6. **Rank-partitioned level transfer.** The replicated gathers cost region
   ...
   overlap margins (order-6 stencils through K stages), which is real
...
9. **Fold-adjacent refinement** stays forbidden, not solved.
   Converging-shock problems want resolution exactly where refinement is
   excluded; the workaround is a globally fine level-0 in r near the fold.
   Lifting it means extending the antipodal pairing across levels — real
...
10. **Distributed `:filter` restriction.** The anti-alias pair is the tool
    ...
    (it has not yet); distributing it means running the fine-side filter
    ...
    before.
11. **Multi-patch I/O.** A patch writes its `region` exactly as a rank
    block does, so the HDF5 hyperslab extension is mechanical; VTK
...
Conservation remains the one standing numerical debt worth restating:
