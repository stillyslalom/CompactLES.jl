...
`:gaussian` sensor smoother, whose rows sit half a cell out at a node-centred
wall, and `ring_sum!`, the `:d8` detector. `wall_closures(scheme, σ)`, in
`kernels.jl` and `kernels_banded.jl`, builds rows for both by folding a
symmetric scheme's interior stencil onto the node-centred mirror of the wall
with the field's sign σ, which inherits a filter's unit row sum and the eighth
derivative's zero row sum. The smoother takes the σ = +1 rows at every face
`sensor_mirror` names, the detector both signs as a pair per dimension that
aliases one plan where neither face is a wall, and a fold's closed far end the
same rows; `planned_sensor_mirror(bc)` gives a `SwitchableBC` face the wall rows
only when both of its conditions are mirrors.

The operator probe is the new `bench/sensorwall.jl`, which applies one operator
on a slab between slip walls and the same operator on the periodic extension of
that slab. Over the six nodes nearest either wall, at N = 49, 97 and 193, both
operators now reproduce the periodic result to round-off, in place of the
smoother's second-order wall defect and the detector's first- and second-order
ones.

At the inviscid slip wall under C6 Brady–Livescu the `detector = :d8` row now
sits below the `:delta4` all-on row, and under `beta_sensor = :dilatation` both
detectors reach the properties-off error, so an inviscid wall under `:d8` is
limited by the strain sensor's cusp as one under `:delta4` is. No smooth-wall
row changes order. In the battery the cold planar Noh wall deficit reads 50%
against 54%, and the aligned Noh case's transverse round-off returns to 7.8e-9,
so its guard returns to the 1e-7 it carried before the detector's wall mirror
widened it; what that transverse mode is remains open. Two `:d8` test guards
moved, since neither the fold field nor u_r = r is a reflection about the outer
slip wall.

Validation: `test/runtests.jl` 2499 of 2499 over 172 testset rows, one row and
20 assertions above the previous count for the new reflecting-wall testset;
`test/convergence.jl` bit-identical; `test/validation.jl` with its header and
the aligned Noh guard re-recorded; and `test/mpi_tests.jl` 302 of 302 at 2 ranks
and 146 of 146 at 8 ranks, all on Julia 1.11.4. An AMD RX 6800 XT run through
AMDGPU.jl agrees bitwise with the `CPUBackend` solver at the wall nodes as in
the interior. The measurements are under
