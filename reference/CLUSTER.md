...
An absolute path is what works. A module sets `PATH` but often not
...
A home directory shared across clusters means one `~/.julia` depot serves every
machine, and no single `LocalPreferences.toml` can be correct for all of them: a
...
Sharing the depot's `compiled/` cache between clusters fails in practice, which is
why the per-architecture depot is standing configuration. The slots should coexist
...
the checkout's; the script lives at the repository root, so locate it through
`pkgdir` rather than a bare path. (The `-e` form loads CompactLES before the
script's `t_start`, so its `pkg load (s)` reads ~0; pass the resolved path as an
argument to measure it.) Run once per machine and whenever a launch flag changes,
and supply the intended grid — it sets the decomposition, so a default-64³ probe
can report floor values that do not apply to the target job.
...
padded-to-interior halo ratio, which is what limits useful decomposition. It
...
measurements before the probe's node-distribution line caught it. Pass
...
The 112 × 2 row is the clean comparison: two threads per rank on verified
two-core masks, `@threaded` fully engaged, lost to pure per-core ranks by ~1.9x
at unchanged rank count. The condensation benefit is real on the communication
side but the thread-side losses consume it. **The verdict is `-t 1` under MPI.**
...
per-region barriers. Trixi.jl, the nearest comparable solver and one that ships a
...
near 112 ranks, and larger counts error rather than degrade.
...
improves performance ~1.7x — memory bandwidth per rank, since the compact line
solves are bandwidth-bound and the decomposition is identical within each pair.
...
- `ThreadPinning`'s querying API drives `clusterprobe.jl`; its *pinning* API is
  ...
  binding was already near-optimal, and the launch-line rules matter more.
