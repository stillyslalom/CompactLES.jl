...
function save_checkpoint(s::Solver, Q, prefix::AbstractString)
    dec = s.dec
    rank = MPI.Comm_rank(dec.comm)
    o1, o2, o3 = dec.Hd
    nx, ny, nz = dec.nloc
    open(_ckpt_name(prefix, rank), "w") do io
        write(io, UInt64(CKPT_MAGIC))
        write(io, Int64(s.ncons))
        for d in 1:3
            write(io, Int64(dec.nglob[d]))
        end
        for d in 1:3
            write(io, Int64(dec.nloc[d]))
        end
        for d in 1:3
            write(io, Int64(dec.coords[d]))
        end
...
function load_checkpoint!(s::Solver, Q, prefix::AbstractString)
    ...
    open(_ckpt_name(prefix, rank), "r") do io
        ...
        Int(read(io, Int64)) == s.ncons || error("conserved layout mismatch")
        for d in 1:3
            Int(read(io, Int64)) == dec.nglob[d] || error("global grid mismatch")
        end
        for d in 1:3
            Int(read(io, Int64)) == dec.nloc[d] || error("decomposition mismatch")
        end
        for d in 1:3
            Int(read(io, Int64)) == dec.coords[d] || error("rank layout mismatch")
        end
        s.t = read(io, Float64)
        s.step = Int(read(io, Int64))
        buf = Array{Float64}(undef, nx, ny, nz, s.ncons)
        read!(io, buf)
...
function _vtk_extent(dec::Decomp)
    lo = ntuple(d -> dec.off[d], 3)
    hi = ntuple(d -> dec.off[d] + dec.nloc[d] - 1, 3)
    lo, hi
end
...
velocity, p, T, and the mass fractions as Float32 point data on the physical
rectilinear grid. Open the .pvtr in ParaView/VisIt.
"""
function save_vtk(s::Solver, Q, prefix::AbstractString)
    dec = s.dec
    rank = MPI.Comm_rank(dec.comm)
    P = MPI.Comm_size(dec.comm)
    nx, ny, nz = dec.nloc
    o1, o2, o3 = dec.Hd
    lo, hi = _vtk_extent(dec)
    ...
    scal("rho", s.rho); scal("p", s.p); scal("T", s.Tt)
    for sp in 1:s.ns
        scal("Y$(sp)", s.Ys[sp])
    end
    ...
    coords = ntuple(d -> Float64[xcoord(s, d, i) for i in 1:dec.nloc[d]], 3)

    fname = string(prefix, ".r", lpad(rank, 4, '0'), ".vtr")
    open(fname, "w") do io
        offs = Int[]
        off = 0
        blk(nbytes) = (push!(offs, off); off += 8 + nbytes)
        for d in 1:3
    ...
    mine = Int64[lo..., hi...]
    allext = MPI.Allgather(mine, dec.comm)
    if rank == 0
        open(string(prefix, ".pvtr"), "w") do io
            glo = ntuple(d -> 0, 3)
            ghi = ntuple(d -> dec.nglob[d] - 1, 3)
            write(io, "<?xml version=\"1.0\"?>\n")
