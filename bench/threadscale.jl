...
@printf("threads=%-3d N=%-4d  primitives!=%9d  deriv=%8d  artificial=%9d  rhs=%9d  step=%10d\n",
        Threads.nthreads(), N,
        probe(() -> CL.primitives!(solver, Q)),
        probe(() -> CL.deriv_along!(solver.tmp_a, solver.rho, solver, 1, 1)),
        probe(() -> CL.compute_artificial!(solver, Q)),
        probe(() -> compute_rhs!(solver, Q, dQ)),
        probe(() -> step!(solver, Q, dQ, du, 1e-4)))
